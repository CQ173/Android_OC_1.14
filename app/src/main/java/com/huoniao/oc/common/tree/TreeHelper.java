...
public class TreeHelper
{
		...
		for (T t : datas)
		{
          ...
          AllTreeNode yy = null;
			...
			for (Field field : fields)
			{
				...
				if(field.getAnnotation(TreeNodeYy.class) != null){
					field.setAccessible(true);
					    yy =(AllTreeNode)field.get(t);
				}
			}
			node = new Node(id, pid, label,yy);
			nodes.add(node);
		}// for end
	...
	private static void setNodeIcon(Node n)
	{
		...
		{
			n.setIcon(R.drawable.tree_right);
		} else if (n.getChildren().size() > 0 && !n.isExpand())
		{
			n.setIcon(R.drawable.tree_down);
		} else
		{
			n.setIcon(-1);
		}
