...
public class Filter implements View.OnClickListener, AdapterView.OnItemClickListener {
    ...
    private LinearLayout linearLayout;  //作为pop为window返回不关闭  有的手机需要有的手机不需要    解决兼容性
    ...
    public Filter(List<FilterDataBean.DataBean> t ){
        ...
        for (FilterDataBean.DataBean filterDataBean: list
             ) {
            if(filterDataBean.isFlag()){
                linList.add(filterDataBean);
            }
        }
    }
    ...
    public void filterState(final Activity activity, View textView){
        myPopWindow = new MyPopAbstract(){
            ...
            protected void setMapSettingViewWidget(View view) {
                ...
                linearLayout = (LinearLayout) view.findViewById(R.id.ll_lin);
        ...
        }.poPwindow(activity,false).showAsDropDown(textView);
    }
                ...
                if(filterLinstener!=null){
                    ...
                    if(linList.size()>0){
                        ...
                        if(stringBuffer.length()>0) {
                            ...
                            filterLinstener.result(substring);
                        }
                    }else{
