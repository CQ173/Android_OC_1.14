...
public class SettingPayPwd extends BaseActivity {
    ...
    private void initData() {
        user = (User) ObjectSaveUtil.readObject(SettingPayPwd.this, "loginResult");
    ...
    private void verificationCode() {
        ...
        if (RegexUtils.checkIdCard(idNum) == false) {
            Toast.makeText(SettingPayPwd.this, "请输入正确的负责人身份证号码!", Toast.LENGTH_SHORT).show();
            return;
        }
        if(newPwd.length()<6){
    ...
    private void startCode(String url, final int tag,String verifyCode) {
        ...
        final JsonObjectRequest jsonObjectRequestUpdatePayPwd = volleyNetCommon.jsonObjectRequest(Request.Method.POST, url, jsonObject, new VolleyAbstract(this) {
            ...
            public void volleyError(VolleyError volleyError) {
                Toast.makeText(SettingPayPwd.this, R.string.netError, Toast.LENGTH_SHORT).show();
            }

            @Override
            protected void netVolleyResponese(JSONObject json) {
                if(tag ==1) {
                    Toast.makeText(SettingPayPwd.this, "获取验证码成功！", Toast.LENGTH_SHORT).show();
                    //获取验证码成功后！进行倒计时
    ...
    private void setUpdatePwd() {
        ...
        JsonObjectRequest jsonObjectRequest = volleyNetCommon.jsonObjectRequest(Request.Method.POST, url, jsonObject, new VolleyAbstract(this) {
            ...
            @Override
            protected void netVolleyResponese(JSONObject json) {
                Toast.makeText(SettingPayPwd.this, "设置支付密码成功！", Toast.LENGTH_SHORT).show();
                MyApplication.payPasswordIsEmpty = false; //表示 用户已经设置过了密码
                finish();
            }
